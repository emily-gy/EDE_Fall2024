...
irradiance.regression <- 
  lm(PeterPaul.chem.nutrients$irradianceWater ~ 
       PeterPaul.chem.nutrients$depth) # x (independent): depth; y (dependent): irradianceWater

# another way to format the lm() function (preferred)
...
2. **Normal Q-Q.** Compare residual to a normal distribution, if residual follow normal distribution, the points should fall close to the 1:1 line. We often see departures from 1:1 at the high and low ends of the dataset, which could be outliers. 
...
par(mfrow = c(2,2), mar=c(4,4,4,4)) # mfrow: dividing the plot screening into 4 cells like a grid; mar: find limit of the grid
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + #set a limit to 2000 to avoid outlier
  geom_point() 
