...
library(ggridges) # package for ridge plot: looks distribution of a variable over time
...
# layout for plots generated in this chunk

# Scatterplot
ggplot(EPAair, aes(x = Date, y = Ozone)) + 
  geom_point()

# generate the same plot in a different way
O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone))
print(O3plot)

# Fix this code (change dots color to blue)
O3plot2 <- ggplot(EPAair) +
  # geom_point(aes(x = Date, y = Ozone, color = "blue")) # however the color is not in blue. when specify color within the aesthetic, it's adding a layer to a plot instead of changing color. correct way:
  geom_point(aes(x = Date, y = Ozone), color = "blue") # correct
print(O3plot2) 

# Add additional variables
# How could you automatically assign a marker color to a variable? 
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) +            # assign different shape and color automatically
  geom_point()
print(PMplot)
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) # Site.Name: row in plot, Year: column in plot
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() # y sacle reversed: 0-20
print(tempvsdepth)

# relationship of tp_ug and yn_ug
NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) +  # lm: linear model, calculate trend line
  geom_abline(aes(slope = 16, intercept = 0)) # already know the slope
print(NvsP)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? color:contour; fill:inside of the boxplot
print(Nutrientplot3)

# Dot plot
Nutrientplot4 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, stackdir = "center", position = "dodge", dotsize = 2) # binaxis: stacking observations over y, position: do not overlay, go to the side
print(Nutrientplot4)

# Violin plot
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_violin(aes(color = nutrient)) # shows density plot
print(Nutrientplot5)
...
Nutrientplot6 <-
  ...
  geom_freqpoly(aes(x = po4), color = "red") #specified variable on x and colors 
print(Nutrientplot6) # no legend; not a good way to generate frequency plot! Just ovelaying on one another

# Using a gathered dataset, where we put all nutrients and concentrations together. Better approach
Nutrientplot7 <-   
...
Nutrientplot6 <-
  ...
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) # categorized y by nutrient: 5 plots. One color for each lake
print(Nutrientplot6)
