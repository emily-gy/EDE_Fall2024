...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_040"]) #p<0.05, reject null
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_041"])
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_046"]) # do not reject
...
shapiro.test(Litter.Totals$dryMass[Litter.Totals$plotID == "NIWO_057"]) # do not reject
...
qqnorm(Litter.Totals$dryMass); qqline(Litter.Totals$dryMass) # does not follow normal distribution

# Test for equal variance
# ANOVA is robust against departures from equal variance (one of the assumptions)
# bartlett.test() performs Bartlett's test of the null that the variances in each of the groups (samples) are the same.
bartlett.test(Litter.Totals$dryMass ~ Litter.Totals$plotID) # p less than 0.05, reject null
#results: reject null i.e. variances are not equal

# Format ANOVA as aov
Litter.Totals.anova <- aov(data = Litter.Totals, dryMass ~ plotID) # specify continuous and categorical variables
...
# Post-hoc test: explore differences between multiple group means
...
Litter.Totals.groups # indicate which groups can have the same mean
...
```{r}
# Format as aov: find relationship between functional group and land cover
