...
# combine vector1 and vector4
c(vector1, vector4)

#assign a new name for new vectors
...
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # filled row by row. if byrow = FALSE, filled by column
matrix2
...
matrix1[4,  ] # 4th row of the matrix

matrix1[ , 3] # 3rd column of the matrix

matrix1[c(12, 14)] # gave positions; brackets are more useful

matrix1[c(12:14)] #

matrix1[2:4, 1:3] # getting matrix from existing matrix
...
```{r Matrix Exercise}
mean(matrix4[ ,'C2']) #use the name of colum 2; or [ ,2]
```
...
# changed the value, make sure to use double brackets
list1[[4]] = 44
```
...
#Combine the vectors into a dataframe: need to have same number of rows
