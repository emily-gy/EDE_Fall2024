...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) # dropped the other lakes
summary(NTL.nutrient.data.PeterPaul$lakename)
...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% # chose the dataset
  mutate(month = month(sampledate)) %>% # find month
  select(lakeid:daynum, month, sampledate:comments) %>% # reorder the column
  drop_na(depth) # drop all the NA
...
# Remove columns that are not of interest for analysis by selecting the columns we want
NTL.phys.data.PeterPaul.subset <- select(NTL.phys.data.PeterPaul, 
                                         lakename:irradianceDeck)
  
NTL.nutrient.data.PeterPaul.subset <- select(NTL.nutrient.data.PeterPaul, lakename, year4, daynum, month, sampledate, depth:po4) # another way to remove unwanted columns
...
# Spread nutrient data back into separate columns using pivot_wider
...
LitterFiles # all 20 files are imported
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth == 0) %>%  #just want surface info
  group_by(lakename, month) %>%  
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% # filter NAs. Using & to ensure none of these are NA are rows
