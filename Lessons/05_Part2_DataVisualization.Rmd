...
O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() # classic looking, no grid line (same as defult)
print(O3plot4)
```
...
# base_size is 11 in dufault ggplot
...
theme_set(mytheme) # set theme for all subsquent plots
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + # horizontal line cross y axis, can be used for define sections
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + # alpha: controls transparency
  # add text, on Jan 1 2020, y at 45. defining 3 different levels
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") + # month represented by abbreviation 
  ylab(expression("O"[3]* " AQI Value")) + # change label using ylab function
  theme(axis.text.x = element_text(angle = 45,  hjust = 1))
print(O3plot7)  # shift x label and angle and horizontal justification
...
# put two lots together
library(cowplot)
plot_grid(NvsP2, Nutrientplot, nrow = 2, align = 'h', rel_heights = c(1.25, 1)) # relative heights
