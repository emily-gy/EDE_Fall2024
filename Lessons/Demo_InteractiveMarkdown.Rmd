...
runtime: shiny # add and save the document
editor_options: 
...
my_plot <- nutrient_data %>% 
  ...
  ggplot(aes(
    x = sampledate,
    y = .data[[the_parameter]], # the correcrt way to add parameter
    color = depth,
    shape = lakename)) +
  geom_point(alpha = 0.4, size = 2) +
  scale_x_date(
...
plot_it <- function(the_parameter, start_year, end_year){ # include all the following
