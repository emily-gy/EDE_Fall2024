...
```{r scrape.some.data}
# create the_registrant, which is from the webpage
the_registrant <- webpage %>%  # go to webpage
  html_nodes(".table tr:nth-child(1) td:nth-child(2)") %>%  #(1)
  html_text()
the_registrant

# (1): specify the html_notes: look at the web and find the ones with ".table tr:nth-child(1) td:nth-child(2)", which is the one we copied from the webpage
```
...
# control & click, make sure the ones we want are in green or yellow
...
avg_withdrawals <- webpage %>% 
  ...
  html_text()
avg_withdrawals # returns character vector (not numeric values, need to convert later)
...
df_withdrawals <- data.frame("Month" = rep(1:12), # create sequence of numbers
                             "Year" = rep(2020,12), # vector of repeat of 2020
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant, # assign variables
...
# following code makes the variables (year, facility) easy to change
...
the_year <- 2015
the_scrape_url <- paste0(the_base_url, '/', the_facility, '/', the_year) # paste0: eliminate adding a space between the elements concatenate into a string
...
scrape.it <- function(the_year, the_facility){
  ...
  # make our code run slowly. 1=1 second, avoid website provider to cut access
...
#1: scrap function, repeat each value in X array and specify the facility

#OR, use purrr's map function (does the same thing as lapply)
the_dfs <- map(the_years,scrape.it,the_facility=my_facility)


#Conflate the returned dataframes into a single dataframe
the_df <- bind_rows(the_dfs) # rbind in tidyverse version
...
#Set the web page (the report page)
...
#Plot
ggplot(df_2020,aes(y = Avg_Withdrawals_mgd, x=Facility_type)) + #x=factor(Month)
  geom_boxplot()
...
the_facilities <- sample(the_facility_ids,3) #random 3 sites
