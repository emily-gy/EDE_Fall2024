...
library(lubridate)
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE) # relative path
...
`>`
`>=`
`&` multiple conditions, both to be true
`|` one or other should be true

```{r filtering data}
# can only compare data with the same type
class(NTL.phys.data$lakeid)
class(NTL.phys.data$depth)

# matrix filtering, look for surface, so look at only the rows has a value of 0 on column depth
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] # nothing after the comma bc we only look at rows

# dplyr filtering
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) # same as matrix filtering
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25) # look at surface less than 0.25 (resulted in the same observations)
...
# Choose multiple conditions to filter for Paul and Peter lakes
summary(NTL.phys.data$lakename) # number of frequency for each lake
# Explore in multiple ways
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") # 1st: used | or cuz wanted either one
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & 
                                     ...
                                     lakename != "Ward Lake" & lakename != "West Long Lake") # 2nd: exclude other lakes
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # 3rd: cleaner and easier


# Choose a range of conditions of a numeric or integer variable (June 1 - October 31)
# by looking at the original data set, we found June 1 is day number 152, October 31 is 304
...
```{r arranging data}
# arrange depth in ascending order
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth)
# arrange depth in descending order
...
```{r selecting columns}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) # : meaning: from column sampledate to temperature_C (this included sampledate, depth, and temperature)
...
```{r mutating data}
# include a new column of temperature in F
...
# reorder columns to put month with the rest of the date variables (moved month to the front, right after daynum)
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments)

# find out the start and end dates of the dataset: interval function
# interval function: create an interval object, specify start and end date
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) #created an interval

# another way to find interval: use function first and function last
...
NTL.phys.data.processed <- 
  NTL.phys.data %>%
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") %>%  # no need to provide dataset name again within pipe
...
```{r exporting data}
# always save the processed data as csv. for further reference
...
# first part of processing is done. Save it, and open another rmd file for the next steps
```
