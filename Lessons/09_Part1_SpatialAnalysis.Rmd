...
GasPlants <- st_read(here('Data/Spatial/Manufactured_Gas_Plant_Sites.geojson')) # just like read.csv
...
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(
    coords = c('mean_Lng','mean_Lat'), # x is longitude, y is latitude
    crs=4326 # crs: coordinate reference system 
    )
...
class(sfc_GraingerHall) #sfc is just a list
mapview(sfc_GraingerHall)

#Upgrade to a spatial dataframe from a list (sfc to sf)
...
the_buffer <- sf_GraingerHall %>% st_buffer(1) # the 1 unit is degree bc crs = 4326 (WGS84), unit in degrees

#View the result
mapview(the_buffer) + mapview(sf_GraingerHall, color = 'red') # why oval? we project a circle on a plat map. if on Google Earth as a sphere, it's gonna be like a circle
```
...
#Transform the datasets to use UTM 17 N, WGS84 (can be found from the epsg website)
...
EPAair.sf.utm$geometry[1] # utm use meters, so geometry column number got larger
# Projected CRS: WGS 84 / UTM zone 17N, why showing WGS 84?
# - because all projected coordinate reference system has an underlying geographic system
...
#Compute the mean of each row (each EPA site) to all different gas sites
