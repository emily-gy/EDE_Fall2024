...
gage_data <- read.table(
  theURL,
  skip = 26, #not reading the first 29 lines
  header=TRUE,
  sep='\t', # separater: tab
  stringsAsFactors = T) 
...
#Load in data using the package's "readNWISdv" function (sepcific for NWIS data)
...
nc_pop <- 
  get_acs(geography = "county", #get_acs is a command from tidycensus
...
#4 Construct a dataframe from the values. the values are all in text for now
df_electricity <- data.frame(States = the_states,
                             Price = the_price, 
                             Capacity = the_capacity,
                             Net_Generation = net_generation,
                             Total_Retail = total_retail)
#we can convert price to number format using as.numeric, but not for others bc of the commas

# convert and plot the function
