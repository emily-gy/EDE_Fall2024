...
epa_pm25_sites_sf <- read_csv(here('Data/Raw/EPAair_PM25_NC2018_raw.csv')) %>% 
  ...
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), crs=4269) #simple feature as spatial feature
...
# If only want to look at epsg
st_crs(epa_pm25_sites_sf)$epsg

#Plot the data
ggplot(data=epa_pm25_sites_sf) +
  geom_sf()
#Color the plot based on meanPM
ggplot(data=epa_pm25_sites_sf) +
...
#Read the 2018 SVI (social vulnerability index) county-level dataset for NC from a website
...
mapview(counties_sf_join, 
        zcol = 'E_POV150', 
        col.regions = brewer.pal(2, 'RdBu')) +  # specify the color intensity and palette 
  mapview(epa_pm25_sites_sf, cex = 'maxPM')  #sized by max PM
...
#View the outputs
mapview(huc8_wind_sf, zcol = 'WindEnergy')
# or with ggplot
ggplot() + 
...
epa_sf_utm <-      st_transform(epa_pm25_sites_sf, crs = 26917) # UTM Zone 17N
...
#IntersetriCo#Intersect the HUC_8s
