...
USGS.flow.data <- read.csv(
  ...
  stringsAsFactors = TRUE)   # all characters are treated as factor, always add this
...
###error: could not find function "home"
...
str(USGS.flow.data) #structure of the dataframe
dim(USGS.flow.data) # dimension, row and column
length(USGS.flow.data) # number of column

head(USGS.flow.data) #first 5 records
head(USGS.flow.data, 10) #first 10
tail(USGS.flow.data, 5) #last 5
USGS.flow.data[30000:30005, c(3, 8, 14)] #only looks at rows 30000 to 30005, columns 3, 8, 14

class(USGS.flow.data$datetime) #looks at specific datetime column
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data)  #could point to column only with $ numeric columns
...
Numeric: would leave as N/A
Character/factor: would leave as blank
...
```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) # removed all the rows with missing values
dim(USGS.flow.data)
dim(USGS.flow.data.complete)

mean(USGS.flow.data$discharge.mean) # mean function output is NA since there are missing values
mean(USGS.flow.data.complete$discharge.mean)
sd(USGS.flow.data.complete$discharge.mean) #standard deviation
...
str(USGS.flow.data)
# here, the field site no. were identified as numeric by R, which is not correct
```
...
```{r}
# therefore we'll convert (coerce) them into factors
USGS.flow.data$site_no <- as.factor(USGS.flow.data$site_no)
str(USGS.flow.data) #the values didn't change, but the data typed have changed
```
...
str(USGS.flow.data2$site_no) # site_no is now factor
```
...
today <- Sys.Date() # created a variable called today. the class is Date
month(today)
day(today)
year(today)
week(today)
...
format(today, format = "%B") #extracted the month from today, unabbreviated format
format(today, format = "%D")
format(today, format = "%m")
format(today, format = "%a")

# convert all the datetime strings/factors into mon
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") # as.Date function converts string/factor to a date format object

USGS.flow.data$datetime[1] #it's 2027-10-01, which is wrong!
...
# create a function using paste0, which concatenate vectors after converting to character
# use ifelse: logical expression, if TRUE, if FALSE
  # earliest date of the function: (19)271001 (max)
  # latest date of the function: (20)191226 (min)
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191226,"19","20"),d) #concatenate 19 if d>191226, otherwise concatenate 20
       })

# use this function to process the dataset again
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)
# now the earliest is 19271001, and latest is 20191226

# convert the datatime into the desired format
