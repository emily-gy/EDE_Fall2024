...
filenames <- dir(here('Data','Raw'),pattern = ("*.csv")) # dir: directory
print(filenames)
```
...
date_obj_today <- ymd(str_today) #year, month, day
date_obj_today # now it's a date object
...
# my exercise
str_past <- "22-feb-3"
fixed_date_obj_past <-ymd(str_past)
fixed_date_obj_past
...
#functions beloe does exact same things as the fix.early.dates fucntion
#Alternative 1
...
```{r}
# convert dow column to factor and explore

#Convert to a factor (grouping variables)
...
ggplot(sites,aes(x=dow)) +
  geom_bar() + 
  scale_x_discrete(drop=FALSE) # if didn't run with previous command, it's gonna drop
